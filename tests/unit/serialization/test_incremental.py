...
def test_serialize_pandas_to_recarray(input_df_descr):
    ...
    if not isinstance(expectation, tuple) and issubclass(expectation, Exception):
        ...
        matching = str(expectation[0]) == str(incr_ser_data)                # FIXME: CM#007 - (deprecated tostring)
        assert matching
...
def test_serialize_incremental_pandas_to_recarray(input_df_descr):
    ...
    if not isinstance(expectation, tuple) and issubclass(expectation, Exception):
        ...
        matching = expectation[0].tobytes() == b''.join(chunk_bytes)        # FIXME: CM#007 - (deprecated tostring)
        assert matching
...
def test_generator_bytes_range(from_idx, to_idx):
    ...
    matching = expectation[0][from_idx:to_idx].tobytes() == b''.join(chunk_bytes)    # FIXME: CM#007 - (deprecated tostring)
    assert matching
    assert expectation[1] == incr_ser.dtype
