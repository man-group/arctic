...
def test_do_db_auth():
    ...
    connection.__getitem__.return_value.authenticate.assert_called_once_with(
...
def test_do_db_auth_no_admin():
    ...
    # Create the user agains the current mongo database
    with patch("arctic.scripts.utils.logger", autospec=True) as logger, patch(
        "arctic.scripts.utils.get_auth", side_effect=[None, user_creds], autospec=True
    ) as get_auth:
...
def test_do_db_auth_no_admin_user_creds_fails():
    connection = MagicMock()
    with patch("arctic.scripts.utils.logger", autospec=True) as logger, patch(
        "arctic.scripts.utils.get_auth", side_effect=[None, None], autospec=True
    ) as get_auth:
        connection.admin.authenticate.return_value = False
        assert not do_db_auth("hostname", connection, "arctic_user")
...
def test_do_db_auth_admin_user_creds_fails():
    connection = MagicMock()
    with patch("arctic.scripts.utils.logger", autospec=True) as logger, patch(
        "arctic.scripts.utils.get_auth", side_effect=[Mock(), None], autospec=True
    ) as get_auth:
        connection.admin.authenticate.return_value = False
        assert not do_db_auth("hostname", connection, "arctic_user")
