...
def test_main():
    with patch("arctic.scripts.arctic_fsck.Arctic") as Arctic, patch(
        "arctic.scripts.arctic_fsck.get_mongodb_uri"
    ) as get_mongodb_uri, patch("arctic.scripts.arctic_fsck.do_db_auth") as do_db_auth:
        run_as_main(
            main,
            "--host",
