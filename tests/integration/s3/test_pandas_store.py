...
def test_write_multi_column(generic_version_store):
    ...
    generic_version_store.write(symbol, df)
    assert np.all(generic_version_store.read(symbol).data == df)


def test_save_read_pandas_series(generic_version_store):
    s = Series(data=[1, 2, 3], index=[4, 5, 6])
    generic_version_store.write('pandas', s)
    saved = generic_version_store.read('pandas').data
    assert np.all(s == saved)
    assert saved.name == "values"


def test_save_read_pandas_series_maintains_name(generic_version_store):
...
def test_save_read_pandas_series_with_multiindex(generic_version_store):
...
def test_save_read_pandas_series_with_multiindex_and_name(generic_version_store):
...
def test_save_read_pandas_series_with_unicode_index_name(generic_version_store):
    df = Series(data=['A', 'BC', 'DEF'],
...
def test_save_read_pandas_dataframe_with_multiindex(generic_version_store):
...
def test_save_read_pandas_dataframe_with_none_values(generic_version_store):
...
def test_save_read_pandas_dataframe_with_unicode_index_name(generic_version_store):
...
def test_save_read_pandas_series_with_datetimeindex_with_timezone(generic_version_store):
...
def test_save_read_pandas_series_with_datetimeindex(generic_version_store):
...
def test_save_read_pandas_dataframe_with_datetimeindex_with_timezone(generic_version_store):
...
def test_save_read_pandas_empty_series_with_datetime_multiindex_with_timezone(generic_version_store):
...
def test_save_read_pandas_dataframe_with_datetimeindex(generic_version_store):
...
def test_save_read_pandas_dataframe_with_strings(generic_version_store):
...
def test_save_read_pandas_dataframe(generic_version_store):
...
def test_save_read_empty_dataframe(generic_version_store):
    df = DataFrame({'a': [], 'b': []})
    generic_version_store.write('pandas', df)
    saved_df = generic_version_store.read('pandas').data
    assert np.all(df.values == saved_df.values)
...
def test_save_read_pandas_dataframe2(generic_version_store):
...
def test_save_read_pandas_dataframe_strings(generic_version_store):
...
def test_save_read_pandas_dataframe_empty_multiindex(generic_version_store):
...
STRATEGY MAC INSTRUMENT CONTRACT $Price $Delta $Gamma $Vega $Theta $Notional uDelta uGamma uVega uTheta Delta Gamma Vega Theta'''),
    ...
    generic_version_store.write('pandas', expected)
    saved_df = generic_version_store.read('pandas').data
    assert np.all(expected.values == saved_df.values)
    assert np.all(expected.index.names == saved_df.index.names)


def test_save_read_pandas_dataframe_empty_multiindex_and_no_columns(generic_version_store):
...
def test_save_read_pandas_dataframe_multiindex_and_no_columns(generic_version_store):
    expected = read_csv(StringIO(u'''\
...
def xtest_append_pandas_multi_columns_dataframe(generic_version_store):
    ...
    generic_version_store.write('test', df)
    generic_version_store.append('test', df2)

    saved = generic_version_store.read('test')
...
def xtest_append_pandas_multi_columns_dataframe_new_column(generic_version_store):
    columns = pd.MultiIndex.from_product([["bar", "baz", "foo", "qux"], ["one", "two"]], names=["first", "second"])
    df = pd.DataFrame(np.random.randn(2, 8), index=[0, 1], columns=columns)
    df2 = pd.DataFrame(np.random.randn(2, 8), index=[2, 3], columns=columns)
    generic_version_store.write('test', df)
...
def test_save_read_pandas_multi_columns_empty_dataframe(generic_version_store):
    columns = pd.MultiIndex.from_product([["bar", "baz", "foo", "qux"], ["one", "two"]], names=["first", "second"])
...
def test_save_read_pandas_multi_columns_dataframe(generic_version_store):
    columns = pd.MultiIndex.from_product([["bar", "baz", "foo", "qux"], ["one", "two"]], names=["first", "second"])
...
def test_save_read_pandas_multi_columns_no_names_dataframe(generic_version_store):
...
def test_save_read_pandas_multi_columns_dataframe_with_int_levels(generic_version_store):
...
def test_save_read_multi_index_and_multi_columns_dataframe(generic_version_store):
    columns = pd.MultiIndex.from_product([["bar", "baz", "foo", "qux"], ["one", "two"]])
...
def xtest_append_pandas_dataframe(generic_version_store):
    df = DataFrame(data=[1, 2, 3], index=DatetimeIndex(start='1/1/2011', periods=3, freq='H'))
    df2 = DataFrame(data=[4, 5, 6], index=DatetimeIndex(start='2/1/2011', periods=3, freq='H'))
    generic_version_store.write('pandas', df)
    generic_version_store.append('pandas', df2)
    saved_df = generic_version_store.read('pandas').data
    assert np.all(df.append(df2).values == saved_df.values)


def test_empty_dataframe_multindex(generic_version_store):
    df = DataFrame({'a': [], 'b': [], 'c': []})
...
def xtest_dataframe_append_empty(generic_version_store):
    df = DataFrame(data=[1, 2, 3], index=DatetimeIndex(start='1/1/2011', periods=3, freq='H'))
...
def xtest_empty_dataframe_append(generic_version_store):
...
def xtest_dataframe_append_empty_multiindex(generic_version_store):
...
def xtest_empty_dataframe_append_multiindex(generic_version_store):
...
def xtest_empty_dataframe_should_ignore_dtype(generic_version_store):
    df = DataFrame({'a': [], 'b': [], 'c': []}).groupby(['a', 'b']).sum()
...
def xtest_empty_dataframe_should_ignore_dtype2(generic_version_store):
    df = DataFrame({'a': []})
    df2 = DataFrame({'a': [1, 1, 1], 'b': [1, 1, 2], 'c': [1, 2, 3]}).groupby(['a']).sum()
    generic_version_store.write('pandas', df)
    generic_version_store.append('pandas', df2)
...
def xtest_dataframe_append_should_promote_string_column(generic_version_store):
    ...
    generic_version_store.write('pandas', df)
    generic_version_store.append('pandas', df2)
    actual = generic_version_store.read('pandas').data

    assert_frame_equal(expected, actual)


def xtest_dataframe_append_should_add_new_column(generic_version_store):
    data = np.zeros((2,), dtype=[('A', 'i4'), ('B', 'f4'), ('C', 'a10')])
    data[:] = [(1, 2., 'Hello'), (2, 3., "World")]
    df = DataFrame(data, index=DatetimeIndex(np.array([dt(2013, 1, 1),
                                                       dt(2013, 1, 2)]).astype('datetime64[ns]'), name=[u'DATETIME']))
...
def xtest_dataframe_append_should_add_new_columns_and_reorder(generic_version_store):
    data = np.zeros((2,), dtype=[('A', 'i4'), ('B', 'f4'), ('C', 'a10')])
    data[:] = [(1, 2., 'Hello'), (2, 3., "World")]
    df = DataFrame(data, index=DatetimeIndex(np.array([dt(2013, 1, 1),
                                                       dt(2013, 1, 2)]).astype('datetime64[ns]'), name=[u'DATETIME']))
...
def test_dataframe_save_read(generic_version_store, df_size):
    df = dataframe(*df_size)
    generic_version_store.write('pandas', df)
    result = generic_version_store.read('pandas').data
...
@pytest.mark.parametrize("df_size", list(itertools.combinations_with_replacement([0, 1, 2, 4], r=3)))
def xtest_dataframe_save_append_read(generic_version_store, df_size):
    df = dataframe(*df_size)
    generic_version_store.write('pandas', df)
    generic_version_store.append('pandas', df)
    result = generic_version_store.read('pandas').data
...
def xtest_large_dataframe_append_rewrite_same_item(generic_version_store):
    csv = \
...
def test_large_dataframe_rewrite_same_item(generic_version_store):
    csv = \
"""index, f1, f2, f3, f4, f5, f6, f7, f8, iVol, tau, uPrice, uDelta, uGamma, uVega, uTheta, Delta, Gamma, Vega, Theta, $Price, $Delta, $Gamma, $Vega, $Theta, $Time_Value, $Notional, FX, f9
0, 201401, 2013 - 12 - 20 16:15:00, 15.0, F1, CALL, STRAT, 140.0, 140.345, 0.07231398622706062, 0.008813407863715872, 0.5768068954653813, 0.6427860135978315, 0.391592427081917, 4.915801583071703, -20.166163353481476, 9.641790203967473, 5.873886406228755, 73.73702374607555, -302.49245030222215, 11909.274289984183, 18625.940769791625, 15925.131550993763, 1014.9606370552315, -1601.4183005499872, 4786.093789984206, 2897689.1805000002, 1.37646, SYM
1, 201401, 2013 - 12 - 20 16:15:00, 15.0, F1, PUT, STRAT, 140.0, 140.345, 0.07231398622706062, 0.008813407863715872, 0.2318116692147143, -0.357200149447554, 0.391592427081917, 4.915801583071703, -20.16670499598669, -5.358002241713311, 5.873886406228755, 73.73702374607555, -302.50057493980034, 4786.192353109285, -10350.550083271604, 15925.131550993763, 1014.9606370552315, -1601.4613130062987, 4786.192353109285, 2897689.1805000002, 1.37646, SYM
...
def xtest_append_after_truncate_after_append(generic_version_store):
    ...
    generic_version_store.write('sym', empty_df)
    full_df = DataFrame(data=[np.zeros(5)], columns=columns)
    generic_version_store.write('sym', full_df)
    generic_version_store.write('sym', empty_df)
    full_df = DataFrame(data=[np.zeros(5)], columns=columns)
    generic_version_store.write('sym', full_df)
    assert len(generic_version_store.read('sym', 1).data) == 0
    assert len(generic_version_store.read('sym', 2).data) == 1
    assert len(generic_version_store.read('sym', 3).data) == 0
    assert len(generic_version_store.read('sym', 4).data) == 1


def test_can_write_pandas_df_with_object_columns(generic_version_store):
    expected = DataFrame(data=dict(A=['a', 'b', None, 'c'], B=[1., 2., 3., 4.]), index=range(4))
    generic_version_store.write('objects', expected)
    saved_df = generic_version_store.read('objects').data
...
def test_panel_save_read(generic_version_store, df_size):
    '''Note - empties are not tested here as they don't work!'''
    pn = panel(*df_size)
    generic_version_store.write('pandas', pn)
    result = generic_version_store.read('pandas').data
...
def test_panel_save_read_with_nans(generic_version_store):
    ...
    generic_version_store.write('pandas', p_in)
    p_out = generic_version_store.read('pandas').data
...
def test_save_read_ints(generic_version_store):
    ts1 = DataFrame(index=[dt(2012, 1, 1) + dtd(hours=x) for x in range(5)],
                    data={'col1':np.arange(5), 'col2':np.arange(5)})
    ts1.index.name = 'index'
    generic_version_store.write('TEST_1', ts1)
    ts2 = generic_version_store.read('TEST_1').data
    assert_frame_equal(ts1, ts2)


def test_save_read_datetimes(generic_version_store):
...
def test_labels(generic_version_store):
...
def test_duplicate_labels(generic_version_store):
    ts1 = DataFrame(index=[dt(2012, 1, 1) + dtd(hours=x) for x in range(5)],
...
def test_no_labels(generic_version_store):
    ts1 = DataFrame(index=[dt(2012, 1, 1) + dtd(hours=x) for x in range(5)],
                    data=[[np.arange(5), np.arange(5, 10)]])
    generic_version_store.write('TEST_1', ts1)
    ts2 = generic_version_store.read('TEST_1').data
...
def test_no_index_labels(generic_version_store):
    ts1 = DataFrame(index=[dt(2012, 1, 1), dt(2012, 1, 2)],
                    data={'data': [1., 2.]})
    generic_version_store.write('TEST_1', ts1)
    ts2 = generic_version_store.read('TEST_1').data
...
def test_not_unique(generic_version_store):
    d = dt.now()
    ts = DataFrame(index=[d, d], data={'near': [1., 2.]})
    ts.index.name = 'index'
    generic_version_store.write('ts', ts)
    ts2 = generic_version_store.read('ts').data
...
def test_daterange_start(generic_version_store):
    df = DataFrame(index=date_range(dt(2001, 1, 1), freq='S', periods=30 * 1024),
                   data=np.tile(np.arange(30 * 1024), 100).reshape((-1, 100)))
    df.columns = [str(c) for c in df.columns]
    generic_version_store.write('MYARR', df)
    ...
    result = generic_version_store.read('MYARR', date_range=DateRange(end=end)).data
    assert len(result) == 1


def xtest_daterange_with_zero_index(generic_version_store):
    ...
    generic_version_store.append('MYARR', df)
    # request for a date range that won't return any values
    result = generic_version_store.read('MYARR', date_range=DateRange(end=dt(2000, 1, 1))).data
    assert len(result) == 0


def test_daterange_large_DataFrame(generic_version_store):
    df = DataFrame(index=date_range(dt(2001, 1, 1), freq='S', periods=30 * 1024),
                   data=np.tile(np.arange(30 * 1024), 100).reshape((-1, 100)))
    df.columns = [str(c) for c in df.columns]
    generic_version_store.write('MYARR', df)
    ...
    # first 100
    result = generic_version_store.read('MYARR', date_range=DateRange(df.index[0], df.index[100])).data
    assert_frame_equal(df[df.index[0]:df.index[100]], result, check_names=False)
    # second 100
    result = generic_version_store.read('MYARR', date_range=DateRange(df.index[100], df.index[200])).data
    assert_frame_equal(df[df.index[100]:df.index[200]], result, check_names=False)
    # first row
    result = generic_version_store.read('MYARR', date_range=DateRange(df.index[0], df.index[0])).data
    assert_frame_equal(df[df.index[0]:df.index[0]], result, check_names=False)
    # last 100
    result = generic_version_store.read('MYARR', date_range=DateRange(df.index[-100])).data
    assert_frame_equal(df[df.index[-100]:], result, check_names=False)
    # last 200-100
    result = generic_version_store.read('MYARR', date_range=DateRange(df.index[-200], df.index[-100])).data
    ...
    # beyond last row
    result = generic_version_store.read('MYARR', date_range=DateRange(df.index[-1], df.index[-1] + dtd(days=1))).data
    assert_frame_equal(df[df.index[-1]:df.index[-1]], result, check_names=False)
    # somewhere in time
    result = generic_version_store.read('MYARR', date_range=DateRange(dt(2020, 1, 1), dt(2031, 9, 1))).data
    assert_frame_equal(df[dt(2020, 1, 1):dt(2031, 9, 1)], result, check_names=False)


def test_daterange_large_DataFrame_middle(generic_version_store):
    df = DataFrame(index=date_range(dt(2001, 1, 1), freq='S', periods=30 * 1024),
                   data=np.tile(np.arange(30 * 1024), 100).reshape((-1, 100)))
    df.columns = [str(c) for c in df.columns]
    generic_version_store.write('MYARR', df)
    ...
    for end in np.arange(200, 30000, 1000):
        result = generic_version_store.read('MYARR', date_range=DateRange(df.index[start], df.index[end])).data
...
def test_daterange(generic_version_store, df, assert_equal):
    df.index.name = 'idx'
    df.name = 'FOO'
    generic_version_store.write('MYARR', df)
    # whole array
    ...
    assert_equal(df, generic_version_store.read('MYARR', date_range=DateRange(df.index[0])).data)
    assert_equal(df, generic_version_store.read('MYARR', date_range=DateRange(df.index[0], df.index[-1])).data)
    assert_equal(df, generic_version_store.read('MYARR', date_range=DateRange()).data)
    assert_equal(df[df.index[10]:], generic_version_store.read('MYARR', date_range=DateRange(df.index[10])).data)
    assert_equal(df[:df.index[10]], generic_version_store.read('MYARR', date_range=DateRange(end=df.index[10])).data)
    assert_equal(df[df.index[-1]:], generic_version_store.read('MYARR', date_range=DateRange(df.index[-1])).data)
    assert_equal(df[df.index[-1]:], generic_version_store.read('MYARR', date_range=DateRange(df.index[-1], df.index[-1])).data)
    assert_equal(df[df.index[0]:df.index[0]], generic_version_store.read('MYARR', date_range=DateRange(df.index[0], df.index[0])).data)
    assert_equal(df[:df.index[0]], generic_version_store.read('MYARR', date_range=DateRange(end=df.index[0])).data)
    assert_equal(df[df.index[0] - DateOffset(days=1):],
                 generic_version_store.read('MYARR', date_range=DateRange(df.index[0] - DateOffset(days=1))).data)
    assert_equal(df[df.index[-1] + DateOffset(days=1):],
                 generic_version_store.read('MYARR', date_range=DateRange(df.index[-1] + DateOffset(days=1))).data)
    assert len(generic_version_store.read('MYARR', date_range=DateRange(dt(1950, 1, 1), dt(1951, 1, 1))).data) == 0
    assert len(generic_version_store.read('MYARR', date_range=DateRange(dt(2091, 1, 1), dt(2091, 1, 1))).data) == 0


def xtest_daterange_append(generic_version_store):
    df = DataFrame(index=date_range(dt(2001, 1, 1), freq='S', periods=30 * 1024),
                   data=np.tile(np.arange(30 * 1024), 100).reshape((-1, 100)))
    df.columns = [str(c) for c in df.columns]
    df.index.name = 'idx'
    ...
    generic_version_store.append('MYARR', rows)
    # assert we can rows back out
    assert_frame_equal(rows, generic_version_store.read('MYARR', date_range=DateRange(rows.index[0])).data)
    # assert we can read back the first array
    assert_frame_equal(df, generic_version_store.read('MYARR', date_range=DateRange(df.index[0], df.index[-1])).data)
    # append two more rows
    rows1 = df.iloc[-2:].copy()
    rows1.index = rows1.index + dtd(days=2)
    generic_version_store.append('MYARR', rows1)
    # assert we can read a mix of data
    assert_frame_equal(rows1, generic_version_store.read('MYARR', date_range=DateRange(rows1.index[0])).data)
    assert_frame_equal(concat((df, rows, rows1)), generic_version_store.read('MYARR').data)
    assert_frame_equal(concat((rows, rows1)), generic_version_store.read('MYARR', date_range=DateRange(start=rows.index[0])).data)
    assert_frame_equal(concat((df, rows, rows1))[df.index[50]:rows1.index[-2]],
                       generic_version_store.read('MYARR', date_range=DateRange(start=df.index[50], end=rows1.index[-2])).data)


def assert_range_slice(generic_version_store, expected, date_range, **kwargs):
    assert_equals = assert_series_equal if isinstance(expected, Series) else assert_frame_equal
    assert_equals(expected, generic_version_store.read('MYARR', date_range=date_range).data, **kwargs)


def test_daterange_single_chunk(generic_version_store):
    ...
    generic_version_store.write('MYARR', df)
    assert_range_slice(generic_version_store, df[dt(2015, 8, 11):], DateRange(dt(2015, 8, 11), dt(2015, 8, 11)))


def test_daterange_when_end_beyond_chunk_index(generic_version_store):
    df = read_csv(StringIO("""2015-08-10 00:00:00,200005,1.0
                              2015-08-10 00:00:00,200012,2.0
                              2015-08-10 00:00:00,200016,3.0
                              2015-08-11 00:00:00,200005,1.0
                  ...
                  names=['date', 'security_id', 'value']).set_index(['date', 'security_id'])
    generic_version_store.write('MYARR', df)
    assert_range_slice(generic_version_store, df[dt(2015, 8, 11):], DateRange(dt(2015, 8, 11), dt(2015, 8, 12)))


def test_daterange_when_end_beyond_chunk_index_no_start(generic_version_store):
    df = read_csv(StringIO("""2015-08-10 00:00:00,200005,1.0
                              2015-08-10 00:00:00,200012,2.0
                              2015-08-10 00:00:00,200016,3.0
                              2015-08-11 00:00:00,200005,1.0
                  ...
                  names=['date', 'security_id', 'value']).set_index(['date', 'security_id'])
    generic_version_store.write('MYARR', df)
    assert_range_slice(generic_version_store, df, DateRange(end=dt(2015, 8, 12)))
...
def test_data_info_series(generic_version_store):
    s = Series(data=[1, 2, 3], index=[4, 5, 6])
    generic_version_store.write('pandas', s)
    md = generic_version_store.get_info('pandas')
...
def test_data_info_df(generic_version_store):
...
def test_data_info_cols(generic_version_store):
    i = MultiIndex.from_tuples([(1, "ab"), (2, "bb"), (3, "cb")])
    s = DataFrame(data=[100, 200, 300], index=i)
    generic_version_store.write('test_data', s)
    md = generic_version_store.get_info('test_data')
...
def test_read_write_multiindex_store_keeps_timezone(generic_version_store):
    ...
    generic_version_store.write('spam', df)
    assert list(generic_version_store.read('spam').data.index[0]) == row0[:-1]
    assert list(generic_version_store.read('spam').data.index[1]) == row1[:-1]


def test_mutable_df(generic_version_store):
    s = DataFrame(data=[1, 2, 3], index=[4, 5, 6])
    s.__array__().setflags(write=True)
    generic_version_store.write('pandas', s)
    read_s = generic_version_store.read('pandas')
