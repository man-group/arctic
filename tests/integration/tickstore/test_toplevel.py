...
def test_should_successfully_do_a_roundtrip_write_and_read_spanning_multiple_underlying_libraries(
    ...
    res = toplevel_tickstore.read(
        "blah", DateRange(start=dt(2010, 12, 1), end=dt(2011, 2, 1)), columns=list("ABCD")
    )
...
def test_should_write_top_level_with_correct_timezone(arctic):
    ...
    expected = pd.DataFrame(
        np.arange(len(dates), dtype=np.float64), index=dates.tz_convert(utc), columns=list("a")
    )
    toplevel_tickstore.write("blah", data)
    res = toplevel_tickstore.read(
...
def test_get_libraries_no_data_raises_exception_tzinfo_given(toplevel_tickstore, arctic):
    ...
    date_range = DateRange(
        start=dt(2009, 1, 1, tzinfo=tzinfo), end=dt(2010, 12, 31, 23, 59, 59, 999000, tzinfo=tzinfo)
    )
    with pytest.raises(NoDataFoundException):
        toplevel_tickstore._get_libraries(date_range)
...
def test_get_library_metadata(arctic):
    ...
    with pytest.raises(NoDataFoundException):
        toplevel_tickstore.read(
            symbol,
            DateRange(start=dt(2010, 1, 1, tzinfo=tzinfo), end=dt(2011, 1, 2, tzinfo=tzinfo)),
            columns=None,
        )
