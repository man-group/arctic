...
def test_init_library(mongo_host):
    ...
    with patch("arctic.scripts.arctic_init_library.do_db_auth", return_value=True), patch(
...
def test_init_library_bad_library(mongo_host):
    with pytest.raises(Exception):
        with patch(
            "arctic.arctic.get_auth", return_value=Credential("admin", "adminuser", "adminpwd", "admin")
        ), patch("pymongo.database.Database.authenticate", return_value=True), patch(
            "argparse.ArgumentParser.error", side_effect=Exception
        ):
            # Create the user agains the current mongo database
