...
def test_prune_versions_symbol(mongo_host, library, library_name):
    with patch("arctic.scripts.arctic_prune_versions.prune_versions", autospec=True) as prune_versions, patch(
        "arctic.scripts.utils.get_auth", return_value=Credential("admin", "adminuser", "adminpwd")
