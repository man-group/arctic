...
#@pytest.mark.skip(reason="DO NOT COMMIT") # TODO DMK
...
def test_daterange_large_DataFrame(library):
    ...
    assert_frame_equal_(df, saved_arr, check_names=False, check_freq=False) # DMK
    # first 100
    result = library.read('MYARR', date_range=DateRange(df.index[0], df.index[100])).data
    assert_frame_equal_(df[df.index[0]:df.index[100]], result, check_names=False, check_freq=False) # DMK
    # second 100
    result = library.read('MYARR', date_range=DateRange(df.index[100], df.index[200])).data
    assert_frame_equal_(df[df.index[100]:df.index[200]], result, check_names=False, check_freq=False) # DMK
    # first row
    result = library.read('MYARR', date_range=DateRange(df.index[0], df.index[0])).data
    assert_frame_equal_(df[df.index[0]:df.index[0]], result, check_names=False, check_freq=False) # DMK
    # last 100
    result = library.read('MYARR', date_range=DateRange(df.index[-100])).data
    assert_frame_equal_(df[df.index[-100]:], result, check_names=False, check_freq=False) # DMK
    # last 200-100
    result = library.read('MYARR', date_range=DateRange(df.index[-200], df.index[-100])).data
    assert_frame_equal_(df[df.index[-200]:df.index[-100]], result, check_names=False, check_freq=False) # DMK
    # last row
    result = library.read('MYARR', date_range=DateRange(df.index[-1], df.index[-1])).data
    assert_frame_equal_(df[df.index[-1]:df.index[-1]], result, check_names=False, check_freq=False) # DMK
    # beyond last row
    ...
    assert_frame_equal_(df[dt(2020, 1, 1):dt(2031, 9, 1)], result, check_names=False, check_freq=False) # DMK
...
def test_daterange_large_DataFrame_middle(library):
    ...
    for end in np.arange(200, 30000, 1000):
        ...
        assert_frame_equal_(df[df.index[start]:df.index[end]], result, check_names=False, check_freq=False) # DMK
    # middle following
...
def test_daterange_append(library):
    ...
    assert_frame_equal_(rows, library.read('MYARR', date_range=DateRange(rows.index[0])).data, check_freq=False) # DMK
    # assert we can read back the first array
    assert_frame_equal_(df, library.read('MYARR', date_range=DateRange(df.index[0], df.index[-1])).data, check_freq=False) # DMK
    # append two more rows
    rows1 = df.iloc[-2:].copy()
    rows1.index = rows1.index + dtd(days=2)
    library.append('MYARR', rows1)
    # assert we can read a mix of data
    assert_frame_equal_(rows1, library.read('MYARR', date_range=DateRange(rows1.index[0])).data, check_freq=False) # DMK
