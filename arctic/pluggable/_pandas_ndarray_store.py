...
from arctic.date._util import to_pandas_closed_closed
from arctic.exceptions import ArcticException
from ._kv_ndarray_store import KeyValueNdarrayStore
...
class PandasStore(KeyValueNdarrayStore):
    ...
    def read(self, backing_store, library_name, version, symbol, date_range=None, **kwargs):
        item = super(PandasStore, self).read(backing_store, library_name, version, symbol,
...
class PandasSeriesStore(PandasStore):
    ...
    def write(self, backing_store, library_name, version, symbol, item, previous_version):
        ...
        super(PandasSeriesStore, self).write(backing_store, library_name, version,
                                             symbol, item, previous_version, dtype=md)

    def append(self, backing_store, library_name, version, symbol, item, previous_version, **kwargs):
        item, md = self.SERIALIZER.serialize(item)
        super(PandasSeriesStore, self).append(backing_store, library_name, version,
                                              symbol, item, previous_version, dtype=md, **kwargs)

    def read(self, backing_store, library_name, version, symbol, **kwargs):
        item = super(PandasSeriesStore, self).read(backing_store, library_name, version,
                                                   symbol, **kwargs)
...
class PandasDataFrameStore(PandasStore):
    ...
    def write(self, backing_store, library_name, version, symbol, item, previous_version):
        item, md = self.SERIALIZER.serialize(item)
        super(PandasDataFrameStore, self).write(backing_store, library_name, version,
                                                symbol, item, previous_version, dtype=md)

    def append(self, backing_store, library_name, version, symbol, item, previous_version, **kwargs):
        item, md = self.SERIALIZER.serialize(item)
        super(PandasDataFrameStore, self).append(backing_store, library_name, version,
        symbol, item, previous_version, dtype=md, **kwargs)

    def read(self, backing_store, library_name, version, symbol, **kwargs):
        item = super(PandasDataFrameStore, self).read(backing_store, library_name, version,
                                                      symbol, **kwargs)
        return self.SERIALIZER.deserialize(item)
...
class PandasPanelStore(PandasDataFrameStore):
    ...
    def write(self, backing_store, library_name, version, symbol, item, previous_version):
        ...
        super(PandasPanelStore, self).write(backing_store, library_name, version, symbol, item, previous_version)

    def read(self, backing_store, library_name, version, symbol, **kwargs):
        item = super(PandasPanelStore, self).read(backing_store, library_name, version, symbol, **kwargs)
